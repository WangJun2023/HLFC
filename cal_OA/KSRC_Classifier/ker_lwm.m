function [Ktrain, Ktest] = ker_lwm(img, rows, cols, trainidx, testidx, wind, sig, filt, blo, mx)
% local weighted mean kernel
% 2012-12-14
...
% local weight
[train_weight, test_weight] = compute_weight;
...
for bi = 1 : block(1), % out block
    ...
    b_trainw = img(:,b_train_widx); b_train_weight = train_weight(:,train_rc);
    for bj = 1 : block(2), % inner block
        ...
        b_testw = img(:,b_test_widx); b_test_weight = test_weight(:,test_rc);
        %disp([b_train_wsize b_test_wsize b_train_size b_test_size]);
        % local weighted mean kernel
        b_Kbuf = kernelmatrix(b_trainw, b_testw, sig);
        b_K = compute_kernel;
        % normalize
        b_s_train_weight = sum(b_train_weight); b_s_test_weight = sum(b_test_weight);
        b_K = bsxfun(@times, b_K, (1./b_s_train_weight)'); b_K = bsxfun(@times, b_K, 1./b_s_test_weight);
...
%==========================================================================
    ...
    function [train_weight, test_weight] = compute_weight
        train_weight = zeros(nWind,train_size); test_weight = zeros(nWind,test_size);
        for b = 1 : block(1),
            [train_rc, con] = block_by_block(b, blo(1), r1_block, c1_block, rows, cols, train_pos);
            if con == 1, continue; end
            [b_train_idx, b_train_size, b_w_train, b_train_widx, b_w_train_size] = set_block_train;
            b_train_weight = zeros(nWind, b_train_size);
            b_train = img(:,b_train_idx); b_trainw = img(:,b_train_widx);
            b_trainbuf = kernelmatrix(b_train, b_trainw, filt);
            for n = 1 : b_train_size,
                x = b_w_train(1:b_w_train_size(n), n);
                KK = b_trainbuf(n, x);
                b_train_weight(1:b_w_train_size(n), n) = KK;
            end
            train_weight(:,train_rc) = b_train_weight;
        end
        clear b_train b_trainw b_trainbuf;
        for b = 1 : block(2),
            [test_rc, con] = block_by_block(b, blo(2), r2_block, c2_block, rows, cols, test_pos);
            if con == 1, continue; end
            [b_test_idx, b_test_size, b_w_test, b_test_widx, b_w_test_size] = set_block_test;
            b_test_weight = zeros(nWind, b_test_size);
            b_test = img(:,b_test_idx); b_testw = img(:,b_test_widx);
            b_testbuf = kernelmatrix(b_test, b_testw, filt);
            for m = 1 : b_test_size,
                x = b_w_test(1:b_w_test_size(m), m);
                KK = b_testbuf(m, x);
                b_test_weight(1:b_w_test_size(m), m) = KK;
            end
            test_weight(:,test_rc) = b_test_weight;
        end
        clear b_test b_testw b_testbuf;
    end
    function b_K = compute_kernel
        if mx == 1,
            b_K = mex_pk_lwm(b_Kbuf, ...
                b_w_train, b_w_train_size, b_train_weight, ...
                b_w_test, b_w_test_size, b_test_weight, ...
                1);
        else
            ...
            for n = 1 : b_train_size,
                ...
                K1 = b_Kbuf(x1,:);
                K1 = bsxfun(@times, K1, b_train_weight(1:b_w_train_size(n), n));
                for m = 1 : b_test_size,
                    x2 = b_w_test(1:b_w_test_size(m), m);
                    K2 = K1(:, x2);
                    K2 = bsxfun(@times, K2, b_test_weight(1:b_w_test_size(m), m)');
                    b_K(n,m) = sum(sum(K2));
                end
            end
        end
